...
io.on('connection', (socket) => {
  ...
  // 🔁 "Next" button: user wants to skip current match and find a new one
  socket.on('next', () => {
  // 1️⃣ Disconnect from current room/partner
  handelDisconnect(socket.id, roomArr, io);

  // 2️⃣ Wait a bit (let disconnection finish)
  setTimeout(() => {
    // 3️⃣ Start matching again
    handelStart(roomArr, socket, () => {}, io);
  }, 300);
  });
